              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                LayoutBuilder(builder: (ctx, constraints) {
                  ...
                  return GridView.count(
                    ...
                    children: [
                      _statCard('Records', _stats['total_records'] ?? 0, icon: Icons.lock),
                      _statCard('TOTP', _stats['total_totp'] ?? 0, icon: Icons.qr_code),
                      _statCard('SSH Keys', _stats['total_ssh'] ?? 0, icon: Icons.key),
                      _statCard('Notes', _stats['total_notes'] ?? 0, icon: Icons.note),
                      _statCard('Tags', _stats['total_tags'] ?? 0, icon: Icons.label),
                      _statCard('API Keys', _stats['total_api_keys'] ?? 0, icon: Icons.vpn_key),
                      _statCard('Sessions', _stats['active_sessions'] ?? 0, icon: Icons.devices),
                      _statCard('Health', '${(_stats['health_score'] ?? 0.0).toStringAsFixed(0)}%',
                        icon: Icons.favorite,
                        color: ((_stats['health_score'] ?? 0.0) as num) >= 80 ? Colors.green : Colors.orange),
                    ],
                  );
                }),
